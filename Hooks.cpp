...
HRESULT __stdcall Hooks::hkD3D11Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (!bOnce)
	{
		...
		if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D11Device), (void **)&Yeet::pDevice)))
		{
			pSwapChain->GetDevice(__uuidof(Yeet::pDevice), (void**)&Yeet::pDevice);
			Yeet::pDevice->GetImmediateContext(&Yeet::pContext);
		}

		ID3D11Texture2D* renderTargetTexture = nullptr;
		if (SUCCEEDED(pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<LPVOID*>(&renderTargetTexture))))
		{
			Yeet::pDevice->CreateRenderTargetView(renderTargetTexture, NULL, &Yeet::pRenderTargetView);
			renderTargetTexture->Release();
		}
	
		ImGui_ImplDX11_Init(hWindow, Yeet::pDevice, Yeet::pContext);
	...
	Yeet::pContext->OMSetRenderTargets(1, &Yeet::pRenderTargetView, NULL);
