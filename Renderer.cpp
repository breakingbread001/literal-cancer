...
void Renderer::Initialize()
{
	...
	m_pTabs = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\arial.ttf", 62.f);
	m_pSubTabs = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\arial.ttf", 25.f);
	//FuckOffKeyState::FuckYou();
...
void Renderer::DrawHealth(const ImVec2& scalepos, const ImVec2& scaleheadPosition, INT8 health, float thickness)
{
	...
	float healthwidth2 = healthwidth1 / 100;
	float defhealthwidth = healthwidth2 * health;

	DrawLine(ImVec2(scalepos.x - width + 5, scaleheadPosition.y), ImVec2(scalepos.x - width + 5, scalepos.y), backcolor, 5.f);
	DrawLine(ImVec2(scalepos.x - width + 5, scalepos.y - defhealthwidth), ImVec2(scalepos.x - width + 5, scalepos.y), color, 5.f);
}
